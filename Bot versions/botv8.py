...
#7. Shows comparision maximum data stored about 2 stocks
@bot.command(name='history_compare',help="displays historical data")
async def history_compare(ctx,stock_name1='TSLA',stock_name2='GE'):
    
    data = yf.download(stock_name1, period="max",interval="1d",group_by="ticker")
    data1 = yf.download(stock_name2, period="max",interval="1d",group_by="ticker")
    # to make use of time
    data.to_csv("stock_data.csv")
    data = pd.read_csv("stock_data.csv")
    data1.to_csv("stock_data.csv")
    data1 = pd.read_csv("stock_data.csv")
    # print(ctx.author) --> important to make a log file
    fig = go.Figure()
    fig.add_scatter(x=data1['Date'], y=data1['Close'],name = stock_name2 )
    fig.add_scatter(x=data['Date'], y=data['Close'],name = stock_name1 )
    fig.update_layout(
        title= 'Historic data between '+stock_name1+' and '+stock_name2,
        xaxis_title="Years",
        yaxis_title="Price",
    )
    
    fig.write_image("historic_compare.png")
    
    await ctx.send(file=discord.File('historic_compare.png'))

# 8. Maximum historic data of one stock
@bot.command(name='history',help="displays information of a stock")
async def history(ctx,stock_name='AAPL'):
    
    data = yf.download(stock_name, period="max",interval="1d",group_by="ticker")
    ...
    fig = px.line(data, x = 'Date', y = 'Close', title=stock_name+ ' Share Prices today')
    fig.write_image("fig1.png")
